...
// Switzerland by default, as 1. that's where I live, and 2. There's some pretty good public transportation here.
let supported_countries = ["CHE"];

// Different 'nodes' and connections between them, more can be added. Nodes were generated by ChatGPT, and edited manually.
let nodes = [
    {
        "id": "zh_air",
        "name": "Zurich Airport",
...
// L is here from LeafletJS. I'll have to implement my own version of this some time...
...
fetch(geoJsonUrl)
    ...
    .then(data => {
        var supportedCountryData = data.features.filter(
            feature => supported_countries.includes(feature.properties.ISO_A3)
        );

        L.geoJSON(supportedCountryData, {
            style: {
                color: "#ff7800",
                weight: 5,
                opacity: 1,
...
// when form submitted, remove drawn paths, and add the shortest path. Add the shortest path to travel options.
document
...
// When path clicked in travel options, open it in full, with all the required data such as QRCodes. In a full release, this is the part where the payment would occur.
